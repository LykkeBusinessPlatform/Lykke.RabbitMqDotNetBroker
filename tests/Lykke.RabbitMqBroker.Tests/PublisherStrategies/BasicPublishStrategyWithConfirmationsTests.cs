...
{
    public class BasicPublishStrategyWithConfirmationsTests
    {
        ...
        [TestCaseSource(nameof(TestCasesWithExchangeType))]
        public void Configure_ConfirmationsAreEnabled(Func<RabbitMqSubscriptionSettings, IRabbitMqPublishStrategy> ctor,
            string exchangeType)
        {
            var settings = RabbitMqSubscriptionSettings.ForPublisher("connectionString", "endpoint");
            var strategy = ctor(settings);
            ...
            channel.Verify(model => model.ConfirmSelect(), Times.Once());
        }
        ...
        [TestCaseSource(nameof(TestCasesWithRoutingKey))]
        public void Publish_WaitsForConfirms(Func<RabbitMqSubscriptionSettings, IRabbitMqPublishStrategy> ctor, 
            string routingKey)
        {
            var settings = RabbitMqSubscriptionSettings.ForPublisher("connectionString", "endpoint");
            var strategy = ctor(settings);
            ...
            channel.Verify(model => model.WaitForConfirmsOrDie(It.IsAny<TimeSpan>()), 
                Times.Once());
        }
        ...
        {
            new object[] {(RabbitMqSubscriptionSettings settings) => new FanoutPublishStrategyWithConfirmations(settings),},
            new object[] {(RabbitMqSubscriptionSettings settings) => new TopicPublishStrategyWithConfirmations(settings),}
        };
        ...
        private static object[] TestCasesWithExchangeType =
        {
            new object[]
            {
                (RabbitMqSubscriptionSettings settings) => new FanoutPublishStrategyWithConfirmations(settings), "fanout",
            },
            new object[]
            {
                (RabbitMqSubscriptionSettings settings) => new TopicPublishStrategyWithConfirmations(settings), "topic",
            }
        };
        ...
        private static object[] TestCasesWithRoutingKey =
        {
            new object[]
            {
                (RabbitMqSubscriptionSettings settings) => new FanoutPublishStrategyWithConfirmations(settings), "",
            },
            new object[]
            {
                (RabbitMqSubscriptionSettings settings) => new TopicPublishStrategyWithConfirmations(settings), "topic-route",
            }
        };
